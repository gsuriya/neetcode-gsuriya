import heapq
import math

class Solution:
    def kClosest(self, points: List[List[int]], k: int) -> List[List[int]]:
        """

        distances = []
        - populate w/ distf from (0,0) for each point

        for dist in distances:
            - add to heap of size k, pop if len(heap) > k
            - max_heap of k smallest values

        return k smallest values left in max_heap

        """
        distances = []
        dist_to_point = defaultdict(list) # distance --> point w/ that distance from origin
        for x, y in points:
            d = math.sqrt(x**2 + y**2) # avoids float precision
            dist_to_point[d].append((x, y))
            distances.append(-d)

        # max_heap with k smallest distances
        max_heap = []
        heapq.heapify(max_heap)

        for d in distances:
            heapq.heappush(max_heap, d)

            while len(max_heap) > k:
                heapq.heappop(max_heap)
        
        # now max_heap has k smallest distances

        res = []
        for d in max_heap:
            # add all points w/ this distance to res
            res.append(dist_to_point[-d].pop())

        return res


